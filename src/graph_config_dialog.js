...
export function GraphConfigurationDialog(dialog, onSave) {
  ...
  function chooseColor(currentSelection) {
    let selectColor = $('<select class="color-picker"></select>');
  ...
  function chooseHeight(currentSelection) {
    ...
    let selectHeight = $('<select class="form-control graph-height"></select>');
    for (let i = 1; i <= MAX_HEIGHT; i++) {
      let option = $("<option></option>").text(i).attr("value", i);
  ...
  function renderSmoothingOptions(elem, flightLog, field) {
    if (elem) {
      ...
      if (field.curve != null) {
        ...
        $("input[name=scale]", elem).val(
          field.curve.outputRange != null
            ? `${(field.curve.outputRange * 100).toFixed(0)}%`
            : `${
                GraphConfig.getDefaultCurveForField(flightLog, field.name)
                  .outputRange * 100
              }%`,
  ...
  function renderField(flightLog, field, color) {
    ...
    // Set the grid state
    $("input[name=grid]", elem).attr(
      "checked",
      field.grid ? field.grid : false,
    );
  ...
  function renderGraph(flightLog, index, graph) {
    ...
    $(".add-field-button", graphElem).click(function (e) {
      fieldList.append(
        renderField(flightLog, {}, GraphConfig.PALETTE[fieldCount++].color),
      );
      e.preventDefault();
    });
    ...
    for (let i = 0; i < graph.fields.length; i++) {
      let field = graph.fields[i],
        fieldElem = renderField(
          flightLog,
          field,
          field.color ? field.color : GraphConfig.PALETTE[i].color,
        );
  ...
  function renderGraphs(flightLog, graphs) {
    let graphList = $(".config-graphs-list", dialog);
  ...
  function populateExampleGraphs(flightLog, menu) {
    ...
    for (i = 0; i < exampleGraphs.length; i++) {
      let graph = exampleGraphs[i],
        li = $('<li><a href="#"></a></li>');

      $("a", li).text(graph.label).data("graphIndex", i);
  ...
  var exampleGraphsButton = $(".config-graphs-add"),
  ...
  exampleGraphsMenu.on("click", "a", function (e) {
    let graph = exampleGraphs[$(this).data("graphIndex")],
