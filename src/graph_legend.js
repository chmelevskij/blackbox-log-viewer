...
export function GraphLegend(
  ...
  function buildLegend() {
    ...
    $(
      ...
      if (!e.altKey) {
        config.selectedFieldName =
          config.getGraphs()[selectedGraphIndex].fields[
            selectedFieldIndex
          ].friendlyName;
        ...
        let graphs = config.getGraphs();
        for (let i = 0; i < graphs[selectedGraphIndex].fields.length; i++) {
          graphs[selectedGraphIndex].fields[i].grid =
  ...
  this.updateValues = function (flightLog, frame) {
    try {
      ...
      $(".graph-legend-field-settings").each(function (index, value) {
        ...
        let str = `Z${(field.curve.outputRange * 100).toFixed(0)} E${(
          field.curve.power * 100
        ).toFixed(0)} S${(field.smoothing / 100).toFixed(0)}`;
        $(this).text(str);
      });
    } catch (e) {
