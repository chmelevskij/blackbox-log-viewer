...
let VIEWER_VERSION = getManifestVersion(); // Current version
...
function BlackboxLogViewer() {
  ...
  function renderSeekBarPicker() {
    let seekBarContainer = $(".seekBar-selection"),
      seekBarPicker,
      seekBarItems = [
    ...
    seekBarPicker.change(function () {
      let activity = flightLog.getActivitySummary(),
  ...
  function renderLogFileInfo(file) {
    ...
    let logIndexContainer = $(".log-index"),
      logIndexPicker,
    ...
    for (index = 0; index < logCount; index++) {
      let logLabel, option, holder, error;
  ...
  function loadFiles(files) {
    for (var i = 0; i < files.length; i++) {
      let isLog = files[i].name.match(/\.(BBL|TXT|CFL|BFL|LOG)$/i),
  ...
  function loadLogFile(file) {
    ...
    reader.onload = function (e) {
      ...
      let fileContents = String.fromCharCode.apply(
        null,
  ...
  $(function () {
    ...
    let logJumpBack = function (fast, slow) {
      ...
      if (hasVideo) {
        ...
        let currentFrame = flightLog.getCurrentFrameAtTime(
    ...
    function expandGraphConfig(index) {
      ...
      let expandedGraphConfig = [];

      for (let i = 0; i < graphConfig[index].fields.length; i++) {
        // Loop through each of the fields
        let singleGraph = { fields: [], label: "", height: 1 };
    ...
    let graphConfigDialog = new GraphConfigurationDialog(
      ...
      exportDialog = new VideoExportDialog($("#dlgVideoExport"), function (
        newConfig,
      ) {
      ...
      if (graph != null && field == null) {
        ...
        for (let i = 0; i < graphConfig[parseInt(graph)].fields.length; i++) {
          if (graphConfig[parseInt(graph)].fields[i].default == null) {
            ...
            graphConfig[parseInt(graph)].fields[i].default.smoothing =
              graphConfig[parseInt(graph)].fields[i].smoothing;
            graphConfig[parseInt(graph)].fields[i].default.outputRange =
              graphConfig[parseInt(graph)].fields[i].curve.outputRange;
            graphConfig[parseInt(graph)].fields[i].default.power =
              graphConfig[parseInt(graph)].fields[i].curve.power;
          }
        }
    ...
    function changePenSmoothing(graphConfig, graph, field, delta) {
      ...
      if (graph != null && field == null) {
        ...
        for (let i = 0; i < graphConfig[parseInt(graph)].fields.length; i++) {
          graphConfig[parseInt(graph)].fields[i].smoothing += delta
            ? -scroll
            : +scroll;
          graphConfig[parseInt(graph)].fields[i].smoothing = constrain(
            graphConfig[parseInt(graph)].fields[i].smoothing,
            range.min,
            range.max,
          );
          changedValue += `${
            graphConfig[parseInt(graph)].fields[i].friendlyName
          } ${(graphConfig[parseInt(graph)].fields[i].smoothing / 100).toFixed(
            2,
          )}%\n`;
        }
        return changedValue;
    ...
    function changePenZoom(graphConfig, graph, field, delta) {
      ...
      const range = { min: 0.1, max: 10.0 }; // 1.0 is actually 100 percent linear!
      const scroll = 0.1;
      ...
      let changedValue = "<h4>Zoom</h4>";
      if (graph != null && field == null) {
        // change ALL pens within group
        for (let i = 0; i < graphConfig[parseInt(graph)].fields.length; i++) {
          graphConfig[parseInt(graph)].fields[i].curve.outputRange += delta
            ? -scroll
            : +scroll;
          graphConfig[parseInt(graph)].fields[i].curve.outputRange = constrain(
            graphConfig[parseInt(graph)].fields[i].curve.outputRange,
            range.min,
            range.max,
          );
          changedValue += `${
            graphConfig[parseInt(graph)].fields[i].friendlyName
          } x${graphConfig[parseInt(graph)].fields[i].curve.outputRange.toFixed(
            1,
          )}\n`;
        }
        return changedValue;
      ...
      if (graph != null && field != null) {
        ...
        graphConfig[parseInt(graph)].fields[
          parseInt(field)
        ].curve.outputRange += delta ? -scroll : +scroll;
        graphConfig[parseInt(graph)].fields[parseInt(field)].curve.outputRange =
          constrain(
            graphConfig[parseInt(graph)].fields[parseInt(field)].curve
              .outputRange,
            range.min,
            range.max,
          );
        return `${
          changedValue +
          graphConfig[parseInt(graph)].fields[parseInt(field)].friendlyName
        } x${graphConfig[parseInt(graph)].fields[
          parseInt(field)
        ].curve.outputRange.toFixed(1)}\n`;
      }
      return false; // nothing was changed
    }
    ...
    function changePenExpo(graphConfig, graph, field, delta) {
      ...
      if (graph != null && field == null) {
        // change ALL pens within group
        for (let i = 0; i < graphConfig[parseInt(graph)].fields.length; i++) {
          graphConfig[parseInt(graph)].fields[i].curve.power += delta
            ? -scroll
            : +scroll;
          graphConfig[parseInt(graph)].fields[i].curve.power = constrain(
            graphConfig[parseInt(graph)].fields[i].curve.power,
            range.min,
            range.max,
          );
          changedValue += `${
            ...
            graphConfig[parseInt(graph)].fields[i].curve.power * 100
          ).toFixed(2)}%\n`;
        }
        return changedValue;
      }
    ...
    $(".log-graph-legend").on("mousedown", function (e) {
      ...
      if (
        ...
        let refreshRequired = restorePenDefaults(
