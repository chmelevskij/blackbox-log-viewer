...
export function GraphConfig(graphConfig) {
  ...
  this.adaptGraphs = function (flightLog, graphs) {
    let logFieldNames = flightLog.getMainFieldNames(),
    ...
    for (let i = 0; i < graphs.length; i++) {
      ...
      for (let j = 0; j < graph.fields.length; j++) {
        ...
        let adaptField = function (field, colorIndexOffset, forceNewCurve) {
          ...
          if (colorIndexOffset != null && field.color != undefined) {
            // auto offset the actual color (to expand [all] selections)
            let index;
            ...
            field.color =
              GraphConfig.PALETTE[
                (index + colorIndexOffset) % GraphConfig.PALETTE.length
              ].color;
          }
          ...
          for (let k = 0; k < logFieldNames.length; k++) {
            if (logFieldNames[k].match(nameRegex)) {
              ...
              let forceNewCurve =
                ...
                nameRoot == "rcCommands" ||
                nameRoot == "debug";
              newGraph.fields.push(
                adaptField(
                  $.extend({}, field, {
                    curve: $.extend({}, field.curve),
                    name: logFieldNames[k],
...
GraphConfig.getDefaultCurveForField = function (flightLog, fieldName) {
  ...
  let getMinMaxForFields = function (/* fieldName1, fieldName2, ... */) {
    ...
    let stats = flightLog.getStats(),
      min = Number.MAX_VALUE,
      max = Number.MIN_VALUE;

    for (let i in arguments) {
      let fieldIndex = flightLog.getMainFieldIndexByName(arguments[i]),
          ...
          switch (fieldName) {
            ...
            case "debug[3]": // After all but Dyn Notch
              return {
                offset: 0,
                power: 0.25 /* Make this 1.0 to scale linearly */,
                inputRange: maxDegreesSecond(
                  gyroScaleMargin * highResolutionScale,
                ), // Maximum grad/s + 20%
                outputRange: 1.0,
              };
            default:
...
GraphConfig.getExampleGraphConfigs = function (flightLog, graphNames) {
  ...
  for (i = 0; i < EXAMPLE_GRAPHS.length; i++) {
    ...
    for (j = 0; j < srcGraph.fields.length; j++) {
      let srcFieldName = srcGraph.fields[j],
        destField = {
          name: srcFieldName,
        };
